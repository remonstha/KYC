...
import UpdateNomineeModal from "../../modal/UpdateModal";
import { useDispatch, useSelector } from "react-redux";
import { setKycSelectionIndividual } from "../../slices/selection.slice";
import { deleteNominee } from "../../slices/kycDetails.slice";
import DeleteIcon from "../../assets/icons/deleteIcon.png";
import UserDetailsCard from "./UserDetailsCard";
import { saveNomineeInfo } from "../../../slices/CentralizedKYC/IndividualKYC/thunk";
...
export default function NomineeForm() {
  ...
  const nomineeInformation = useSelector((state) => state.IndKyc?.nomineeInformation);

  console.log("nominee ID ", nomineeInformation);
  ...
  const handleNext = () => {
    ...
    const updatedValues = {
      nomineeInfos:   initialValues.map((item) => ({
      
       ...item,
    })),
  
    videoKycId: videoKYC?.id,
    id: nomineeInformation.id,
  }
    console.log('hiiiiiiibbbb', updatedValues)
    dispatch(saveNomineeInfo(updatedValues))
  }
          ...
          <div className="row py-0">
            ...
            >
              <button
                ...
                onClick={toggle}
                style={{ color: "#2E5AAA", cursor: "pointer", borderRadius: "2px 0px 0px 2px" }}
              >
                Add New Nominee
              </button>
            </div>
                  ...
                  <Card key={index} style={cardStyle}>
                    ...
                    >
                      <img
                        ...
                        onClick={() => dispatch(deleteNominee(index))}
                      />
                    </div>
                    <Col md={3} style={div1style}>
                      ...
                      <div className="mt-2 text-center ">
                        <p style={{ marginBottom: "4px" }}>{initialValues.name}</p>
                        ...
                        <p style={{ marginBottom: "4px" }}>
                          {initialValues.permDistrict}, {initialValues.permCountry}
                        </p>
                        <p
                          style={{
                    ...
                    <Col className="mt-3">
                      <div>
                        <Row className="ms-2 mt-2 p-2">
                          ...
                          <Col xs="7" className="mb-2">
                            {" "}
                            <strong>{initialValues.tempDistrict}, {initialValues.tempCountry}</strong>
                          </Col>
                        </Row>
                      </div>
                    </Col>
