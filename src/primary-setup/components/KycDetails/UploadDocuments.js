...
import { updateUploadDocuments } from "../../slices/kycDetails.slice";
import { postUploadDocuments } from "../../../slices/CentralizedKYC/IndividualKYC/thunk";
...
export default function UploadDocuments() {
  ...
  const initialValues = useSelector(
    (state) => state.KycDetails.uploadDocuments
  );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(setKycSelectionIndividual("FormVerification"));

      var formData = new FormData();
      
      formData.append("fileVm[0].Files", values.applicantPassportSizePhoto, values.applicantPassportSizePhoto.name);
      formData.append("fileVm[0].FileType", 1);

      formData.append("fileVm[1].Files", values.applicantCitizenshipFrontPhoto, values.applicantCitizenshipFrontPhoto.name);
      formData.append("fileVm[1].FileType", 2);

      formData.append("fileVm[2].Files", values.applicantCitizenshipBackPhoto, values.applicantCitizenshipBackPhoto.name);
      formData.append("fileVm[2].FileType", 3);

      formData.append("fileVm[3].Files", values.signatureOfAccountHolder, values.signatureOfAccountHolder.name);
      formData.append("fileVm[3].FileType", 4);

      formData.append("fileVm[4].Files", values.applicantPassportFront, values.applicantPassportFront.name);
      formData.append("fileVm[4].FileType", 5);

      formData.append("fileVm[5].Files", values.applicantPassportBack, values.applicantPassportBack.name);
      formData.append("fileVm[5].FileType", 6);

      formData.append("fileVm[6].Files", values.nomineePassportSizePhoto, values.nomineePassportSizePhoto.name);
      formData.append("fileVm[6].FileType", 7);

      formData.append("fileVm[7].Files", values.rightThumbPrint, values.rightThumbPrint.name);
      formData.append("fileVm[7].FileType", 8);

      formData.append("fileVm[8].Files", values.leftThumbPrint, values.leftThumbPrint.name);
      formData.append("fileVm[8].FileType", 9);

      formData.append("fileVm[9].Files", values.nomineeIdentificationFront, values.nomineeIdentificationFront.name);
      formData.append("fileVm[9].FileType", 10);

      formData.append("fileVm[10].Files", values.nomineeIdentificationBack, values.nomineeIdentificationBack.name);
      formData.append("fileVm[10].FileType", 11);

      if(videoKYC?.id !== undefined) {
        formData.append("VideoKycId", videoKYC?.id)
      }

      dispatch(postUploadDocuments(formData));
    },
  });

  const [applicantPassportSizePhoto, setApplicantPassportSizePhoto] = useState(
    reduxApplicantPassportSizePhoto || null
  );
  const [applicantCitizenshipFrontPhoto, setApplicantCitizenshipFrontPhoto] =
    useState(reduxApplicantCitizenshipFrontPhoto || null);
  const [signatureOfAccountHolder, setSignatureOfAccountHolder] = useState(
    reduxSignatureOfAccountHolder || null
  );
  const [applicantCitizenshipBackPhoto, setApplicantCitizenshipBackPhoto] =
    useState(reduxApplicantCitizenshipBackPhoto || null);
  const [applicantPassportFront, setApplicantPassportFront] = useState(
    reduxApplicantPassportFront || null
  );
  const [applicantPassportBack, setApplicantPassportBack] = useState(
    reduxApplicantPassportBack || null
  );
  const [nomineePassportSizePhoto, setNomineePassportSizePhoto] = useState(
    reduxNomineePassportSizePhoto || null
  );
  const [rightThumbPrint, setRightThumbPrint] = useState(
    reduxRightThumbPrint || null
  );
  const [leftThumbPrint, setLeftThumbPrint] = useState(
    reduxLeftThumbPrint || null
  );
  const [nomineeIdentificationFront, setNomineeIdentificationFront] = useState(
    reduxNomineeIdentificationFront || null
  );
  const [nomineeIdentificationBack, setNomineeIdentificationBack] = useState(
    reduxNomineeIdentificationBack || null
  );

  useEffect(() => {
    formik.setFieldValue(
      "applicantPassportSizePhoto",
      applicantPassportSizePhoto
    );
    formik.setFieldValue(
      "applicantCitizenshipFrontPhoto",
      applicantCitizenshipFrontPhoto
    );
    formik.setFieldValue(
      "applicantCitizenshipBackPhoto",
      applicantCitizenshipBackPhoto
    );
    ...
    formik.setFieldValue(
      "nomineeIdentificationFront",
      nomineeIdentificationFront
    );
    formik.setFieldValue(
      "nomineeIdentificationBack",
      nomineeIdentificationBack
    );
  }, [
    applicantPassportSizePhoto,
    applicantCitizenshipFrontPhoto,
    applicantCitizenshipBackPhoto,
    signatureOfAccountHolder,
    applicantPassportFront,
    applicantPassportBack,
    nomineePassportSizePhoto,
    rightThumbPrint,
    leftThumbPrint,
    nomineeIdentificationFront,
    nomineeIdentificationBack,
      ...
      <CardHeader className="px-4">
        ...
        <CardBody className="px-4">
          ...
          <Col md={12}>
            ...
            >
              Note: Click on the box or drag and drop the files to upload the
              documents.
            </div>
          </Col>
          ...
          >
              ...
              >
                ...
                {reduxApplicantPassportFront || applicantPassportFront ? (
                  <>
                    <img
                      src={URL.createObjectURL(
                        applicantPassportFront || reduxApplicantPassportFront
                      )}
                      alt="Applicant's Passport Front"
                      style={{
                        opacity: "0.7",
                        objectFit: "cover",
                        width: "100%",
              ...
              >
                ...
                {reduxApplicantPassportBack || applicantPassportBack ? (
                  <>
                    <img
                      src={URL.createObjectURL(
                        applicantPassportBack || reduxApplicantPassportBack
                      )}
                      alt="uploaded"
                      style={{
                        opacity: "0.7",
              ...
              >
                ...
                {reduxRightThumbPrint || rightThumbPrint ? (
                  <>
                    <img
                      src={URL.createObjectURL(
                        rightThumbPrint || reduxRightThumbPrint
                      )}
                      alt="uploaded"
                      style={{
                        opacity: "0.7",
              ...
              >
                ...
                {reduxLeftThumbPrint || leftThumbPrint ? (
                  <>
                    <img
                      src={URL.createObjectURL(
                        leftThumbPrint || reduxLeftThumbPrint
                      )}
                      alt="uploaded"
                      style={{
                        opacity: "0.7",
            ...
            <Col sm={12} className="d-flex justify-content-between mt-4">
              <Button
                color="light"
                onClick={() =>
                  dispatch(setKycSelectionIndividual("Recommender"))
                }
              >
                Back
              </Button>
