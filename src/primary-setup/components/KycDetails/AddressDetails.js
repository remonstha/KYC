...
import { updateAddressDetails, updateUserCountry } from "../../slices/kycDetails.slice";
...
import { saveAddressDetails } from "../../../slices/CentralizedKYC/IndividualKYC/reducer";
import { saveAddressInfo } from "../../../slices/CentralizedKYC/IndividualKYC/thunk";
...
const AddressDetails = () => {
  ...
  const initialValues = useSelector((state) => state.KycDetails.addressDetails);
  const adressID = useSelector((state) => state.IndKyc?.addressDetails);
  console.log("adressID ID", adressID?.id)
  ...
  const formik = useFormik({
    ...
    // validationSchema,
    onSubmit: (values) => {
      const selectedPermCountryLabel = countries.find(
        (country) => country.value === values.permCountry
      )?.label;

      dispatch(updateUserCountry(selectedPermCountryLabel))
      ...
      const valueToSave = {
        permanentAddress: {
          ...
          district: values.permDistrict,
          municipality: values.permMunicipality,
          wardNo: values.permWardNo,
          toleStreet: values.permToleStreet,
          houseNo: values.permHouseNo,
        },
        isSameAsPermanent: true,

        temporaryAddress: {
          country: values.tempCountry,
          state: values.tempState,
          district: values.tempDistrict,
          municipality: values.tempMunicipality,
          wardNo: values.tempWardNo,
          toleStreet: values.tempToleStreet,
          houseNo: values.tempHouseNo,
        },
        officeAddress: {
          country: values.officeCountry,
          state: values.officeState,
          district: values.officeDistrict,
          municipality: values.officeMunicipality,
          wardNo: values.officeWardNo,
          toleStreet: values.officeToleStreet,
          houseNo: values.officeHouseNo,
        },
      ...
      dispatch(saveAddressInfo(valueToSave));
      console.log("Address Details", valueToSave);
              ...
              <FormGroup>
                ...
                {formik.touched.officeDistrict &&
                  selectTouched.officeDistrict === false && (
                    <p className="text-danger">{formik.errors.officeDistrict}</p>
                  )}
              </FormGroup>
            </Col>
            <Col md={4}>
              <FormGroup>
                ...
                <Select
                  ...
                  options={municipalities}
                  value={municipalities?.find(
                    (option) => option.value === formik.values.officeMunicipality
                  )}
                  onChange={(value) => {
            ...
            <Col md={4}>
              <FormGroup>
                ...
                {formik.touched.officeToleStreet && (
                  <p className="text-danger">{formik.errors.officeToleStreet}</p>
                )}
              </FormGroup>
            </Col>
