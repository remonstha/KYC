...
import Table from "../KycDetails/table";
...
import { updateDeclarationMinor } from "../../slices/KycMinor.slice";

export default function DeclarationMinor() {
  const dispatch = useDispatch();
  const [visiable, setVisiable] = useState(true);

  const initialValues = useSelector((state) => state.KycMinor.declaration);
  const tableValues = useSelector((state) => state.KycMinor.declaration.memberInfo);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const newValue = {
          ...values, memberInfo : tableValues,
        
      }
      dispatch(updateDeclarationMinor(newValue));
      console.log("values", newValue);
      dispatch(setKycSelectionMinor("NomineeMinor"))   
     },
  });
