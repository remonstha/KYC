...
import { updateContactInformationMinor } from "../../slices/KycMinor.slice";
...
const validationSchema = Yup.object().shape({
    ...
    residenceLandline: Yup.string().required('Landline No. (Residence) is required'),
    officeLandline: Yup.string().required('Landline No. (Office) is required'),
    email: Yup.string().email('Invalid email').required('Email is required'),
    socialMedia: Yup.string().required('Social Media Username and Type is required'),
    nonResName: Yup.string().required('Name (Non-Resident) is required'),
    nonResMobileNo1: Yup.string().required('Mobile/Phone No. (Non-Resident) is required'),
    address1: Yup.string().required('Address 1 (Non-Resident) is required'),
    relationship1: Yup.string().required('Relationship 1 (Non-Resident) is required'),
    landlineNo1: Yup.string().required('Landline No. 1 (Non-Resident) is required'),
    nonResName2: Yup.string().required('Name 2 (Non-Resident) is required'),
    nonResMobileNo2: Yup.string().required('Mobile/Phone No. 2 (Non-Resident) is required'),
    address2: Yup.string().required('Address 2 (Non-Resident) is required'),
    relationship2: Yup.string().required('Relationship 2 (Non-Resident) is required'),
    landlineNo2: Yup.string().required('Landline No. 2 (Non-Resident) is required'),
...
export default function ContactMinor() {
 ...
 useEffect(() => {
  dispatch(fetchAccountTypeList());
    ...
    const initialValues = useSelector(state => state.KycMinor.contactInformation);

    const formik = useFormik({
        initialValues,
        // validationSchema,
        onSubmit: values => {
            dispatch(updateContactInformationMinor(values));
            dispatch(setKycSelectionMinor('Guardian'))        
            ...
            <Form onSubmit={formik.handleSubmit} className="row gx-5 gy-4">
              ...
              <Col md={4} className="faxNumber">
                ...
                <Input
                  ...
                  onBlur={ formik.handleBlur}
                />
                {formik.errors.faxNumber && formik.touched.faxNumber &&  (
              ...
              {/*---------------------------------- Other Bank Details -------------------–––---------*/}
              <Col lg={12} className="otherBanks mt-5 pt-3">
                <p className="pt-3 fs-16 fw-semibold ">Other Bank Details</p>
              </Col>
              ...
              <Button color="light" onClick={() => dispatch(setKycSelectionMinor('AddressDetailsMinor'))}>Back</Button>
